...
def test__determine_change_handle_valid_input_event(
    ...
    data_stream = environment.from_collection([change_message.to_json()])
...
def test__determine_change_handle_invalid_input_event(
    environment: StreamExecutionEnvironment,
) -> None:
    """
    Test the handling of invalid input data by `DetermineChange`.

    Asserts
    -------
    - The output should contain exactly one `ValidationError`.
    """
    ...
    output = list(determine_change.validation_erorrs.execute_and_collect())
